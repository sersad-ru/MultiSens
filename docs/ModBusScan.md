...
* The maximum number of devices to find is stored in `SCAN_MAX` constant (see [plgModBusScan.cpp](/src/plgModBusScan.cpp)).
* The device reply timeout (in milliseconds) is stored in `TIMEOUT_MS` constant (see [plgModBusScan.cpp](/src/plgModBusScan.cpp)).
