...
namespace HC06 {
  ...
  uint8_t _try_command(SoftwareSerial &ser, char* buf, const char* cmd, const char* line_end=NULL){
    ser.print(cmd);    
  ...
  // Определяем linvor
  uint8_t _check_linvor(SoftwareSerial &ser, char* buf){
  ...
  uint8_t _send_cut_print(SoftwareSerial &ser, char* buf, const char* cmd, const char* cut, const char* pref){
  ...
  // Определяем HC-05/06
  uint8_t _check_hc(SoftwareSerial &ser, char* buf){
  ...
  // Определяем JDY-31
  uint8_t _chech_jdy(SoftwareSerial &ser, char* buf){
    if(!_send_cut_print(ser, buf, "AT+NAME", "+NAME=", "")) return 0;
  ...
  void _check_device(SoftwareSerial &ser, char* buf){
